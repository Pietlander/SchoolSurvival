...
func generate_terrain() -> void:
	...
	for x in range(-half_width, half_width):
		for y in range(-half_height, half_height):
			...
			# 1. Add EVERY cell to the floor list so Godot doesn't leave gaps
			all_floor_cells.append(current_cell)
			
			# 2. Sort out which cells are walls and which are the inside floor
	...
	# Step 1: Draw the floor EVERYWHERE (This prevents the 1-tile gap!)
	set_cells_terrain_connect(all_floor_cells, terrain_set_index, floor_terrain_index, true)
	
	# Step 2: Overwrite the outer edge with your Wall tile
	for cell in wall_cells:
		set_cell(cell, wall_source_id, wall_atlas_coords)
	
	# Step 3: Randomly rotate ONLY the inside floor tiles
